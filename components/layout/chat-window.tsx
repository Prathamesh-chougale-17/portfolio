...
import type { ChatMessage, ConversationItem } from "./chat-types";
...
export default function ChatWindow({ onClose }: Props) {
  ...
  const handleSubmit = async (e?: React.SyntheticEvent) => {
    ...
    const conversation: ConversationItem[] = [
      ...
      ...messages.map((m) => ({
        role: m.role as ConversationItem["role"],
        content: m.content,
      })),
      { role: userMessage.role, content: userMessage.content },
    ];

    await sendMessageMutation.mutateAsync({ messages: conversation, locale });
  };
    ...
    >
      ...
      <div className="flex items-center justify-between border-b p-3">
        ...
        <Actions>
          ...
          <Action onClick={onClose} tooltip={t.chat.close}>
            <X className="h-4 w-4" />
          </Action>
        </Actions>
      </div>
